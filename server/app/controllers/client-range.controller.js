...
const getClientRanges = async (req, res) => {
  ...
  try {
    const dbResponse = await db.query(
      `select cr.id, cr.procedure_id marker_id, c.name marker_name, cr.seq, cr.compare_item, cr.compare_operator,
    ...
    left join marker c on c.id=cr.procedure_id
...
const resetClientRange = async (req, res) => {
  ...
  try {
    ...
    const insertResponse = await db.query(`insert into client_range
      select null, ${req.client_id}, procedure_id marker_id, seq, compare_item, compare_operator, compare_to, range_low, range_high, now(), ${req.user_id}, now(), ${req.user_id}
      from client_range 
      where client_id=1`);
    await db.query(
...
const updateClientRange = async (req, res) => {
  ...
  try {
    ...
    $sql = `update client_range set procedure_id='${marker_id}', range_low=${range_low}, range_high=${range_high}`;
...
const getClientRange = async (req, res) => {
  ...
  try {
    const dbResponse = await db.query(`
      select cr.procedure_id marker_id, c.name marker_name, cr.seq, cr.compare_item, cr.compare_operator, cr.compare_to, cr.range_low, cr.range_high
...
const createClientRange = async (req, res) => {
  ...
  client_range.procedure_id = marker_id;
