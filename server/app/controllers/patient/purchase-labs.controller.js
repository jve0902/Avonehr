...
const getPurchaseLabs = async (req, res) => {
  ...
  try {
    const $sql = `select pc.id patient_cpt_id, c.id proc_id, c.name cpt_name, c.price, pc.created, lc.name lab_company_name
    from patient_cpt pc
...
const createPurchaseLabs = async (req, res) => {
  ...
  try {
    ...
    const intentData = {
      ...
      customer: formData.customer_id,
      description: `${JSON.stringify(formData.patient_cpt_ids)}; patient_id: ${req.user_id
        }`,
